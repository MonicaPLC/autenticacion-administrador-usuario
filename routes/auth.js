...
router.get("/login", (req, res) => {
  ...
  if (req.session.user != null) {
    req.flash(
      "errors",
      "Tu ya estas logeado en el sistema. Para salir tienes que cerrar sesión."
    );
...
router.post("/register", async (req, res) => {
  ...
  if (usuarios.count == 0) tipo_usuario = "ADMIN";
  ...
  try {
    ...
    req.flash(
      "mensajes",
      "El usuario " + user.name + " fue creado correctamente."
    );
  } catch (err) {
